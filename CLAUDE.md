...
- Vite + React 19 + TypeScript + Tailwind v4 (matches `webvybe/agency-site`
  and `webvybe/demo-salon` conventions, using the same button/nav-link component
...
Wellness (salon) demo, since this is this client's own identity. No hardcoded
...
- Gallery/hero/etc. imagery is now real, licensed stock photography (Pexels,
  free-for-commercial-use, no attribution required) generic to the bakery
  vertical, not real photos of a real Wildflour Bakehouse location -- see
